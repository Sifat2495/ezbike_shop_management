...
class _DuePageState extends State<DuePage> {
  ...
  StreamSubscription<DocumentSnapshot>? customerDueListener; 
  ...
  void initState() {
    ...
    if (userId != null) {
      _loadCustomers(userId, isInitialLoad: true);
      ...
      builder: (context, snapshot) {
        ...
        if (_searchText.isNotEmpty) {
          customers = customers.where((customer) {
            ...
            var phone = customerData['phone']?.toString().toLowerCase() ?? ''; 
        ...
        if (_selectedFilter == 'অটোর খাতা') {
          customers = customers.where((customer) {
            ...
            return customerData['chassis'] != null &&
                customerData['chassis'].isNotEmpty;
          }).toList();
        } else if (_selectedFilter == 'ব্যাটারির খাতা') {
          customers = customers.where((customer) {
            var customerData = customer.data();
            return customerData['chassis'] == null ||
                customerData['chassis'].isEmpty;
          }).toList();
        } else if (_selectedFilter == '২০২২') {
          customers = customers.where((customer) {
            var customerData = customer.data();
            if (customerData['time'] != null) {
              DateTime timestamp = customerData['time'].toDate(); 
      ...
      body: Column(
        children: [
          ...
          Container(
            ...
            child: Center(
              child: Text(
                "মোট বাকিঃ ${totalDueAmount.toStringAsFixed(0)} টাকা",
                style: TextStyle(
                  color: Colors.black,  
                  fontSize: 18,
                  fontWeight: FontWeight.bold,
  ...
  Widget _buildCustomerTile(
    ...
    String imageUrl = customerData?['image'];
    String name = customerData?['name'] ?? 'Unknown';
    List<String> phoneNumbers = List<String>.from(customerData?['phones'] ?? []);
    String phone = customerData?['phone'] ?? 'Unknown';
    String presentAddress = customerData?['present_address'] ?? '';
    DateTime? txnTime = customerData?['time'] != null
        ? (customerData?['time'] as Timestamp).toDate()
        : null;
    String customerId = customer.id;
    double customerDue = (customerData?['customer_due'] as num?)?.toDouble() ?? 0.0;
    bool status = customerDue <= 0;
    ...
    return Padding(
      ...
      child: Container(
        decoration: BoxDecoration(
          color: status ? Colors.blue[700] : Colors.red, 
        ...
        child: ListTile(
          leading: GestureDetector(
            onTap: () {
              _showImageBySize(context, imageUrl, customer);
            },
            child: ClipRRect(
          ...
          trailing: Row(
            ...
            children: [
              Text(
                '৳ ${convertToBengaliNumbers((customerData?['customer_due'] ?? '0').toStringAsFixed(0))}',
                style: TextStyle(
                  color: Colors.yellowAccent, 
                  fontWeight: FontWeight.bold,
                  fontSize: 18,
                ...
                child: PopupMenuButton<String>(
                  ...
                  onSelected: (value) {
                    if (value == 'Image Collection') {
                      ...
                      _dueCallection(context, customer);
                    } else if (value == 'Delete') {
                      _showDelete(context, userId, customer);
  ...
  Future<void> _scheduleNotification(
      DateTime scheduledDate, String customerName, double dueAmount) async {
    try {
      ...
      await userNotificationsRef.add({
        'title': 'কিস্তি আদায়',
        'description':
        '$customerName এর বাকি পরিমাণ: ৳${dueAmount.toStringAsFixed(0)}, আজকে কিস্তি কালেকশনের দিন।',
  ...
  void _dueCallection(BuildContext context, DocumentSnapshot customer) {
    Map<String, dynamic>? customerData = customer.data() as Map<String, dynamic>?;
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (customerData?['image'] != null && customerData!['image'].isNotEmpty)
                        ClipRRect(
                          borderRadius: BorderRadius.circular(10.0),
                          child: Image.network(
                            customerData!['image'],
                      ...
                      Text(
                        'মোট বাকি: ৳${convertToBengaliNumbers((customerData?['customer_due'] ?? '0').toStringAsFixed(0))}',
                        style: TextStyle(
                          color: Colors.red,
                          fontSize:
                            ...
                            if ((double.tryParse(amountController.text) ?? 0) > 0 ||
                              ...
                              if (amount > 0.0 || discount > 0.0) {
                                try {
                                  ...
                                  final previousDue = customerData?['customer_due'] ?? 0.0;
                                  ...
                                  await customer.reference.update({'customer_due': updatedDue});
                                  ...
                                  await FirebaseFirestore.instance
                                    ...
                                    'amount': amount,
                                    'reason':
                                    'কিস্তি আদায়ঃ "${customerData?['name']}" নামের কাস্টমার $amount টাকা বাকি পরিশোধ করেছেন।',
                                          ...
                                          child: AlertDialog(
                                            ...
                                            content: SingleChildScrollView(
                                              child: Column(
                                                ...
                                                children: [
                                                  Text(
                                                    'কিস্তি আদায়: ৳${amount.toStringAsFixed(0)}',
                                                    style: TextStyle(
                                                      fontSize:
                                                      screenWidth * 0.04,
  ...
  void sendSMSToCustomer(String phoneNumber, String message) async {
  ...
  Future<void> _pickAndUploadImage(
    ...
    try {
      ...
      final File compressedFile = File('${tempDir.path}/${customer.id}.jpg');
      ...
      String fileName =
          'collection name/${getCurrentUserId()}/customer_images/${customerData?['name']}${customerData?['phone']}.jpg';
  ...
  void _showDelete(
    ...
    double transactionAmount = customer['customer_due'] ?? 0;
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
            '${customer['name']} - এর সাথে $transactionAmount টাকা লেনদেন রয়েছে।\n'
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
              'আপনি কি ${customer['name']} নামের কাস্টমারকে ডিলিট করতে চান? আপনি যদি ডিলিট করেন তাহলে ${customer['name']}-এর পূর্বের সকল বাকির হিসাব মুছে যাবে।'),
          actions: [
            ElevatedButton(
              style: ElevatedButton.styleFrom(
                ...
                if (isDelete) {
                  ...
                  if (pinVerified) {
                    try {
                      ...
                      String? imageUrl = customer['image'];
                      ...
                      await deleteSaleData(customer.id);
