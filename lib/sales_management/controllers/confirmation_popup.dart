...
class ConfirmationPopup extends StatefulWidget {
  ...
  const ConfirmationPopup({
    ...
    required this.time, required this.color, required this.battery,
  });
...
class _ConfirmationPopupState extends State<ConfirmationPopup> {
  ...
  bool _isSaleButtonEnabled = false;
  ...
  Future<void> _scheduleNotification(DateTime scheduledDate, String customerName, double dueAmount) async {
    try {
      ...
      await userNotificationsRef.add({
        'title': 'বাকি আদায়',
        'description': '$customerName এর বাকি পরিমাণ: ৳${dueAmount.toStringAsFixed(0)}, আজকে কিস্তি পরিশোধ করবেন।',
  ...
  void _calculateRemainingAmount() {
    ...
    setState(() {
      ...
      _installmentAmount = months > 0 ? _remainingAmount / months : 0.0;
      _isSaleButtonEnabled = downPayment <= _totalPrice && downPayment >= 0 &&
          (downPayment == _totalPrice || _monthController.text.isNotEmpty);
    });
  }
  ...
  Future<String?> _uploadCustomerImage(String imagePath) async {
    if (imagePath.isEmpty) return null;

    File imageFile = File(imagePath);
    String userId = FirebaseAuth.instance.currentUser!.uid;
    String fileName = '${widget.name}_${widget.phone}_${DateTime.now().millisecondsSinceEpoch}.jpg';
    String filePath = 'collection name/$userId/customer_images/$fileName';
  ...
  void _handleSale() async {
    if (!_isSaleButtonEnabled) return;

    setState(() {
      _isSaleButtonEnabled = false;
    });

    showLoadingDialog(context, message: 'বিক্রি করা হচ্ছে...');

    double payment = double.tryParse(_downPaymentController.text) ?? 0.0;
    double months = double.tryParse(_monthController.text) ?? 0.0;
    double due = _totalPrice - payment;
    ...
    if (widget.imagePath.isNotEmpty) {
      imageUrl = await _uploadCustomerImage(widget.imagePath);
    }

    List<Map<String, String>> updatedGuarantors = await _uploadGuarantorPhotos(widget.guarantors);

    await _saleStockService.saveSaleData(
      name: widget.name,
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
                ...
                Column(
                  ...
                  children: [
                    ...
                    Text('মাতা: ${widget.motherName}',
                        style: TextStyle(fontSize: screenWidth * 0.035)),
                    Text(
                      'তারিখ: ${DateFormat('dd-MM-yyyy hh:mm a').format(widget.birthDate)}',
                      style: TextStyle(fontSize: screenWidth * 0.035),
                    ),
                    ...
                    Text('অটোর চেসিস নম্বর: ${widget.chassis}',
                        style: TextStyle(fontSize: screenWidth * 0.035)),
                    Text('কালার: ${widget.color}',
                        style: TextStyle(fontSize: screenWidth * 0.035)),
                    Text('ব্যাটারি: ${widget.battery}',
                        style: TextStyle(fontSize: screenWidth * 0.035)),
            ...
            Center(
              child: Text(
                '${_monthController.text} কিস্তি = ৳${formatter.format(_installmentAmount)}/মাস',
                style: TextStyle(
              ...
              children: [
                      ...
                      child: TextFormField(
                        ...
                        inputFormatters: [
                          ...
                          TextInputFormatter.withFunction((oldValue, newValue) {
                            ...
                            if (value != null && value <= _totalPrice) {
                              return newValue;
                            }
                            return oldValue;
                          }),
                ...
                SizedBox(
                  ...
                  child: TextFormField(
                    ...
                    enabled: double.tryParse(_downPaymentController.text) !=
                        _totalPrice,
                  ),
                ),
              ],
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                Expanded(
                  child: ElevatedButton(
                    onPressed: _isSaleButtonEnabled ? _handleSale : null,
                    style: ElevatedButton.styleFrom(
                      backgroundColor:
                      _isSaleButtonEnabled ? Colors.green : Colors.grey,
                      shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(
                          MediaQuery.of(context).size.width * 0.03,
                        ),
